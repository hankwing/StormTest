 ...
 */
public class JoinWindowCounter<T> implements Serializable {

  private static final long serialVersionUID = -2645063988768785810L;

  private JoinBasedCounter<T> objCounter;
  private int headSlot;
  private int tailSlot;
  private int windowLengthInSlots;

  public JoinWindowCounter(int windowLengthInSlots) {
    ...
    this.objCounter = new JoinBasedCounter<T>(this.windowLengthInSlots);

    this.headSlot = 0;
    this.tailSlot = slotAfter(headSlot);
  }

  public void addToSlot(T obj, GlobalStreamId id, Tuple tuple) {
    objCounter.addToSlot(obj, headSlot, id, tuple);
  }
   ...
   */
  public Map<T, Map<GlobalStreamId, List<Tuple>>> getRemoveTuplesThenAdvanceWindow() {
    Map<T, Map<GlobalStreamId, List<Tuple>>> counts =  
    		objCounter.getRemoveTuplesAndWipeSlot(tailSlot);

    return counts;
  }

  public void resetAndadvanceHead() {
	  
	objCounter.resetSlot(tailSlot);
    headSlot = tailSlot;
